...
static uint32_t DAP_JTAG_IDCode(const uint8_t *request, uint8_t *response) {
  ...
  printf("JTAG_ReadIDCode:0x%x",data);
  // Store Data
  *(response+0) =  DAP_OK;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
extern void CAN_ReConfig(int32_t baudRate,int32_t filterID);
      ...
      case 0x80:{
          *response = 0x80;
          switch(*request++){
              ...
              case 0x01:{//配置CAN指定波特率
                  void CAN_ReConfig(int32_t baudRate,int32_t filterID);
              }break;
              case 0x02:{//关闭CAN
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
uint8_t dapIsConnected = false;
uint32_t DAP_ProcessCommand(const uint8_t *request, uint8_t *response) {
  uint32_t num;
  PRINTF_THIS("%s->%s:0x%02x\r\n",__filename(__FILE__),__func__,*request);
  ...
  *response++ = *request;
  ...
  switch (*request++) {
   ...
   PRINTF_THIS("%s->%s: num=%d\r\n",__filename(__FILE__),__func__,num);
...
uint32_t DAP_ExecuteCommand(const uint8_t *request, uint8_t *response) {
  uint32_t cnt, num, n;
PRINTF_THIS("%s->%s\r\n",__filename(__FILE__),__func__);
  if (*request == ID_DAP_ExecuteCommands) {//0x7F
    *response++ = *request++;
    cnt = *request++;
