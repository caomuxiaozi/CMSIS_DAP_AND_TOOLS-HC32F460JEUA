...
#define DAP_ThreadId 0
static volatile int osFlags;
...
//发送数据
extern void USBD_HID_GetReportTrigger(int a, int b, void* report, int len);
...
bool USBD_HID0_SetReport(uint8_t rtype, uint8_t req, uint8_t rid, const uint8_t* buf, int32_t len) {
  ...
  case HID_REPORT_OUTPUT:
    ...
    if((uint16_t)(USB_RequestCountI - USB_RequestCountO) == DAP_PACKET_COUNT) {
      USB_EventFlags |= 0x80;//osThreadFlagsSet(DAP_ThreadId, 0x80U);
    ...
    USB_RequestCountI++;
    USB_EventFlags |= 0x01;//osThreadFlagsSet(DAP_ThreadId, 0x01U);
    break;
  case HID_REPORT_FEATURE:
    break;
  }
...
volatile int8_t canUsbSendIndex = 0;
extern usb_core_instance  usb_dev;
bool usrRcvFlag = false;
extern uint32_t usbGetReportLast;
...
//    }
  if((USB_EventFlags & 0x81) != 0) {
    ...
    /*if*/while(USB_RequestCountI != USB_RequestCountO) {
