...
void SGUI_ItemsBase_Repaint(SGUI_SCR_DEV* pstDeviceIF, SGUI_ITEMS_BASE* pstObj)
{
    ...
    static SGUI_POINT              stItemTextPos;
    ...
    if((NULL != pstDeviceIF) && (NULL != pstObj))
    {
        ...
        if(pstObj->iCount > 0)
        {
            ...
            if(pstPaintingItem==ITEMS_SENECT_ITEM(pstObj)){//添加滚动效果
              uint8_t strlength = strlen(cszItemText);
              if(strlength > 18){
                pstPaintingItem->xOffset-=(LIST_SCROOL_SPEED);
                if(pstPaintingItem->xOffset/pstObj->pstFontRes->iHalfWidth < 15 - strlength){
                  pstPaintingItem->xOffset = 5*pstObj->pstFontRes->iHalfWidth;
                }
                stItemTextPos.iX = pstPaintingItem->xOffset;
              }
            }
            ...
            if(pstObj->iVisibleItems > 1)
            {
                ...
                while((pstPaintingItem != ITEMS_VISIBLE_END_ITEM(pstObj)) && (NULL != pstPaintingItem))
                {
                  ...
                  if(pstPaintingItem==ITEMS_SENECT_ITEM(pstObj)){
                    ...
                    if(strlength > 18){
                      pstPaintingItem->xOffset-=(LIST_SCROOL_SPEED);
                      if(pstPaintingItem->xOffset/pstObj->pstFontRes->iHalfWidth < 10 - strlength){
                        pstPaintingItem->xOffset = 5*pstObj->pstFontRes->iHalfWidth;
                      }
                      stItemTextPos.iX = pstPaintingItem->xOffset;
                    }
