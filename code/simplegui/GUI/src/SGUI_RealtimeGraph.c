...
void SGUI_RealtimeGraph_Repaint(SGUI_SCR_DEV* pstDeviceIF, SGUI_RTGRAPH* pstRTGraph)
{
    ...
    if((NULL != pstRTGraph) && (NULL != pstDeviceIF))
    {
        ...
        if(SGUI_TRUE == pstRTGraph->stControl.EnableBaseline)
        {
            ...
            SGUI_Basic_DrawHorizontalLine(pstDeviceIF, SGUI_RECT_X_START(pstRTGraph->stLayout), SGUI_RECT_X_END(pstRTGraph->stLayout), iBaseLineCoordinateY, SGUI_COLOR_DOT);
        }
        ...
        if(pstRTGraph->stData.stRecorder.iSize > 1)
        {
            ...
            {
//              SGUI_Basic_DrawPoint(pstDeviceIF,iValueIndex,pstRTGraph->stData.stRecorder.arrValue[iValueIndex].iYCoord,SGUI_COLOR_FRGCLR);
...
//        }
        pstRTGraph->stData.stRecorder.arrValue[iValueIndex].iYCoord = SGUI_RealtimeGraph_GetValuePointYCoordinate(pstRTGraph, pstRTGraph->stData.stRecorder.arrValue[iValueIndex].iValue);
        }
    }
}
