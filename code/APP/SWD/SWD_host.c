...
#define Flash_Page_Size FlashDeviceInfo.szPage
...
// size is in bytes.
uint8_t swd_read_block(uint32_t address, uint8_t* data, uint32_t size) {
...
uint8_t swd_read_core_register(uint32_t n, uint32_t* val) {
  unsigned int /*i = 0, */timeout = getTick() + FlashDeviceInfo.toErase + FlashDeviceInfo.toProg;
...
uint8_t swd_wait_until_halted(void) {
  ...
  uint32_t val, /*i, */timeout = getTick() +  FlashDeviceInfo.toProg + FlashDeviceInfo.toErase;
  while(getTick() < timeout) {
...
// SWD Switch
uint8_t swd_switch(uint16_t val) {
  uint8_t tmp_in[2];
  tmp_in[0] = val & 0xff;
  tmp_in[1] = (val >> 8) & 0xff;
...
//    uint32_t tmp = 0;
  ...
  if(!swd_read_idcode(&dap_state.IDCODE)) {
    return 0;
  }
...
uint8_t swd_init_debug(void) {
  uint32_t tmp = 0;
//    int i = 0;
  unsigned int timeout = getTick() + FlashDeviceInfo.toErase + FlashDeviceInfo.toProg;
